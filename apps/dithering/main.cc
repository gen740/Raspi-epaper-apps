...
//       auto index = 3 * (j * WIDTH + i);
//       if (canvas[index] == 0 && canvas[index + 1] == 0 &&
//           canvas[index + 2] == 0) {
//         std::print("0, ");
//       } else {
//         std::print("1, ");
...
auto main(int argc, char **argv) -> int {
  ...
  int iw, ih, ic;
  uint8_t *input = stbi_load(argv[1], &iw, &ih, &ic, 3);
  if (!input) {
    std::cerr << "Failed to load image\n";
    return 2;
  }
  ...
  for (size_t i = 0; i < WIDTH; ++i) {
    for (size_t j = 0; j < HEIGHT; ++j) {
      // auto index = 3 * (j * WIDTH + i);
      // std::print("{:#5F}, ",
      //            static_cast<float>(canvas[index] + canvas[index + 1] +
      //                               canvas[index + 2]) /
      //                (3 * 255));
    }
    // std::print("\n");
  ...
  if (!stbi_write_png(argv[2], WIDTH, HEIGHT, 3, canvas.data(), WIDTH * 3)) {
    std::cerr << "Failed to write image\n";
    return 3;
  }
